...
class DDPM(nn.Module):
    def __init__(self, nn_model, betas, n_T, device, n_classes = 20, drop_prob=0.1):
...
def train_mnist():
    ...
    dataset = StateTransitionsDataset('/home/punygod_admin/pgm/pgm_project/c-swm/data/shapes_train.h5')
    ...
    for ep in range(n_epoch):
        ...
        for x, c, x1 in pbar:
            optim.zero_grad()
            # print(x.shape, c.shape, x1.shape)
            print(c.min(), c.max())
            x = x.to(device)
            c = c.to(device)
            x1 = x1.to(device)
        ...
        with torch.no_grad(): 
            ...
            for w_i, w in enumerate(ws_test):
                x_gen, x_gen_store = ddpm.sample(n_sample, (3, 50, 50), device, guide_w=w)
